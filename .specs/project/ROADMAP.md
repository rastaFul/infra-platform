...
- [x] **Promtail: logs PM2 → Docker socket** — DONE 2026-08-29, `features/observability-promtail-docker/spec.md`. Todos os 4 produtos (rastafinancas, microgrow, vetcare, artists-booking) com Promtail via `docker_sd_configs`, validado com Loki query real. 2 bugs reais achados: telegraf 403 (token de segurança não propagado), self-scraping do promtail em vetcare (filtro sem âncora). Batch 3 do infra-platform: 100% fechado.
...
- [x] `ansible-lint` — desbloqueado E fechado 2026-09-16 (máquina só tinha Python 3.8, `ansible-lint`
  precisa 3.10+). Python 3.12.14 instalado via `uv` (sem sudo), `ansible-lint` 26.8.0 via `uv tool
  install`. Rodado de verdade pela primeira vez: 29 achados reais em 14/18 arquivos — **todos
  corrigidos** (roles `credentials-check`/`agents-harness` renomeados sem hífen, var-naming
  prefixado por role, `mode: preserve` em vez de modo fixo em syncs de diretório misto,
  `ignore_errors` trocado por `stat`+`when`, instalador do nvm sem pipe `curl|bash`, 2 `noqa`
  documentados para exceções deliberadas — ver DECISIONS.md/execution.md pro detalhe completo).
