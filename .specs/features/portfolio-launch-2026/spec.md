...
| `rastafinancas` | **PRIVATE** (correção: esta linha dizia PUBLIC originalmente, ficou desatualizada — status real e atual é PRIVATE, ver detalhe) | achado crítico não previsto: `apps/api/.env` real (JWT_SECRET, VAPID, Resend, Google/GitHub OAuth secrets) também estava no histórico, não só o `.env.e2e` original. Purgado via `git filter-repo`. Depois de tornar público, o commit antigo continuou acessível por SHA direto (comportamento de cache do GitHub) — **repo inteiro apagado e recriado do zero** (limpeza 100% confirmada, commit antigo retorna 404). Usuário decidiu não rotacionar as 4 credenciais agora e manter o repo **privado** até rotacionar — decisão final, não temporária. |
